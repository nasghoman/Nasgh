...
<body>
  ...
  <div class="card">
    ...
    <div class="subtitle">اتصال مباشر مع المسبار عبر Bluetooth Low Energy</div>
  ...
  <div class="card">
    <div class="ai-title">توصية نَسغ بالذكاء الاصطناعي (Gemini)</div>
  ...
  <script>
    // ========== إعدادات BLE (نفس اللي في كود ESP32) ==========
    const SERVICE_UUID        = "6e400001-b5a3-f393-e0a9-e50e24dcca9e";
    const CHARACTERISTIC_UUID = "6e400003-b5a3-f393-e0a9-e50e24dcca9e";

    let device = null;
    let characteristic = null;
    let lastData = null; // نخزن آخر JSON مستلم من العصا
    ...
    async function connect() {
      if (!navigator.bluetooth) {
        ...
        alert("Web Bluetooth غير مدعوم في هذا المتصفح. جرب Google Chrome على أندرويد أو على الحاسوب.");
        return;
      }
      ...
      try {
        ...
        // نخلي المتصفح يعرض كل الأجهزة، ونختار المسبار يدويًا
    ...
    function handleNotification(event) {
      ...
      try {
        ...
        lastData = data; // نخزن آخر قراءة

        const now = new Date();
        readingMetaEl.textContent = "آخر قراءة من: " + (data.id || "مسبار غير معروف");
    ...
    // ========== استدعاء توصية AI من Vercel ==========
    // على Vercel، الـ API موجودة تحت نفس الدومين: /api/nasgh-ai
    const AI_API_URL = "/api/nasgh-ai";

    async function callAI() {
      if (!lastData) {
        aiStatusEl.textContent = "لا توجد قراءة حديثة لإرسالها. الرجاء تشغيل المسبار وأخذ قراءة أولاً.";
        return;
      }

      aiBtn.disabled = true;
      aiStatusEl.textContent = "جاري إرسال البيانات وتحليلها...";
      aiAdviceEl.textContent = "جاري التواصل مع نموذج الذكاء الاصطناعي (Gemini)...";
